...
(define-box om-sudoku-csp ((list list))
  ...
  :icon 150
  :doc ""
  (andv (notv (memberv nil (mapcar (lambda (x) (apply #'/=v x)) (rows list))))
        (notv (memberv nil (mapcar (lambda (x) (apply #'/=v x)) (cols list))))
        (notv (memberv nil (mapcar (lambda (x) (apply #'/=v x)) (chunks list))))))
...
(defun sudoku2 (s)
  (let ((v (mapcar (lambda (x)
    ...
    (evaluatev3 v 
               (lambda (y)
                 (andv (notv (memberv nil (mapcar (lambda (x) (apply #'/=v x)) (rows y))))
                       (notv (memberv nil (mapcar (lambda (x) (apply #'/=v x)) (cols y))))
                       (notv (memberv nil (mapcar (lambda (x) (apply #'/=v x)) (chunks y))))))
               r))
  v)
